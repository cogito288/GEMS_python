            ...
            for utc=0:7
                ...
                if np.all(stn_6km)==0 # no observation data in some utc
                    ...
                    if high_rate_pre<30 & low_rate_pre<30 & size(data_stn(:,)>1,1)>1000
                        ...
                        data_stn(data_stn(:,-1)==1 & data_stn(:,)<=8 & data_stn(:,24)>60 & data_stn(:,24)<240,:) = []
                    ...
                    if high_rate_pre>30
                        ...
                        data_stn(data_stn(:,)==1 & data_stn(:,-1)==1 & data_stn(:,24)>=240,:)=[]
                    
                    
                    if low_rate_pre>30
                        print (('low_rate_pre = #3.2f & remove stacked samples \n',low_rate_pre)
                        data_stn(data_stn(:,)==1 & data_stn(:,-1)==1 & data_stn(:,24)<=60,:) = []
                    ...
                    if high_rate<30 &low_rate>30
                        ...
                        if np.all(low)==0
                            idx_low = np.where(data_stn(:,26) <= low(,26) & data_stn(:,24)<=60 & data_stn(:,)==1)
                            data_stn(idx_low,:) = []
                        ...
                        if np.all(high)==0
                            idx_high = np.where(data_stn(:,26) <= high(,26) & data_stn(:,24)>=240 & data_stn(:,)==1)
                            data_stn(idx_high,:) = []
