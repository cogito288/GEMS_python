        ...
        if doy <=30 && yr == 2015:
            for utc in range(7):
                ...
                if np.all(stn_6km==0): # no observation data in some utc
                    ...
                    ndata_7 = oversampling_sh(data_7[:,:6],stn_6km_location,lon_goci.shape[0],lon_goci.shape[1],28]
            ...
            if doy >= 30 & utc == 7:
                ...
                temp_df.to_csv(os.path.join(path_save,'time_conc','dataset', 'PM25',fname), float_format='%7.7f')
                print (doy)
        else:
            for utc in range(7):
                k=240               
                ...
                if not np.all(data_stn==0): # notempty
                   None # Do nothing
                elif data_stn[data_stn[:,28]==1,:].shape[0]>1:
                    rmovr = random.sample(data_stn[data_stn[:,-1]==1,:], np.round(data_stn(data_stn[:,-1]==1,:].shape[0]*0.99))
                    data_stn[data_stn[:,-1]==1,:]=[]
                ...
                if np.all(stn_6km==0): # no observation data in some utc
                    stn_idx = matlab.ismember(stn_6km_location[:,1], stn_6km[:,12])
                    stn_conc = stn_6km[:,[10,12,0,4,1]] # PM10, stn_num, doy_num,time,yr
                    stn_conc[:,5]=0# PM10, stn_num, doy_num,time,yr, ovr
                    stn_conc[:,6] =stn_6km_location[stn_idx,4]# PM10, stn_num, doy_num,time,yr, ovr, stn_location
                    ...
                    data_2 = stn_conc[stn_conc[:,0]>20 &stn_conc[:,0]<=40,:]   # (x0.3)
                    data_3 = stn_conc[stn_conc[:,0]>40 &stn_conc[:,0]<=60,:]   # (x0.5)
                    data_4 = stn_conc[stn_conc[:,0]>60 & stn_conc[:,0]<=120,:]  # (x1)
                    data_5 = stn_conc[stn_conc[:,0]>120 & stn_conc[:,0]<=240,:]  # (x3)
                    data_6 = stn_conc[stn_conc[:,0]>240 & stn_conc[:,0]<=360,:]  # (x13)
                    data_7 = stn_conc[stn_conc[:,0]>360 & stn_conc[:,0]<=480,:]  # (x30)
                    data_8 = stn_conc[stn_conc[:,0]>480,:]  # (x37)
                    ...
                    ndata_5 = oversampling_sh(data_5[:,:6],stn_1km_location,lon_kor.shape[0],lon_kor.shape[1],2]
                    ndata_6 = oversampling_sh(data_6[:,:6],stn_1km_location,lon_kor.shape[0],lon_kor.shape[1],12]
                    ndata_7 = oversampling_sh(data_7[:,:6],stn_1km_location,lon_kor.shape[0],lon_kor.shape[1],28]
                    ndata_8 = oversampling_sh(data_8[:,:6],stn_1km_location,lon_kor.shape[0],lon_kor.shape[1],36]
                    ...
                    if high_rate_pre<30 & low_rate_pre<30 & (data_stn[:,-1]>1).shape[0]>10000:
                        ...
                        data_stn[data_stn[:,-1]==1 & data_stn[:,-1]<=8 & data_stn[:,23]>60 & data_stn[:,23]<240,:] = []
                    ...
                    if high_rate_pre>30:
                        ...
                        data_stn[data_stn[:,]==1 & data_stn[:,-1]==1 & data_stn[:,23]>=240,:]=[]
                    
                    if low_rate_pre>30:
                        print ('low_rate_pre = {low_rate_pre:3.2f} & remove stacked samples \n')
                        data_stn[data_stn[:,]==1 & data_stn[:,-1]==1 &data_stn[:,23]<=60,:] = []
                    ...
                    if high_rate<30 &low_rate>30:
                        ...
                        if np.all(low==0):
                            idx_low = np.where(data_stn[:,25] <= low[-1,25] & data_stn[:,23]<=60 & data_stn[:,]==1)[0] # remove the low concentration samples under than doy
                            data_stn[idx_low,:] = []
