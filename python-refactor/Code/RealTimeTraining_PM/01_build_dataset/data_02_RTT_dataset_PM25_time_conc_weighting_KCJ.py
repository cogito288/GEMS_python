        ...
        if doy <=30:
            ...
            for utc in range(7):
                ...
                if len(stn_6km)!=0: # no observation data in some utc
                    ...
                    data_1 = stn_conc[stn_conc[:,0]<=20,:]            # (x0.01)
                    ...
                    data_8 = stn_conc[(stn_conc[:,0]>480),:]  # (x3)
                    ...
                    if data_1.shape[0]>1:
                        mask = np.random.choice(data_1.shape[0], np.round(data_1.shape[0]*0.1).astype(int), replace=False)
                        data_1 = data_1[mask, :]
                    if data_2.shape[0]>1:
                        mask = np.random.choice(data_2.shape[0], np.round(data_2.shape[0]*0.3).astype(int), replace=False)
                        data_2 = data_2[mask, :]                        
                    if data_3.shape[0]>1:
                        mask = np.random.choice(data_3.shape[0], np.round(data_3.shape[0]*0.5).astype(int), replace=False)
                        data_3 = data_3[mask, :]

                    nndata = np.concatenate((data_1,data_2,data_3,data_4,data_5,data_6,data_7,data_8), axis=0)
                    
                    ndata_5 = matlab.oversampling_sh(data_5[:,:6],stn_6km_location,lon_goci.shape[0],lon_goci.shape[1],2,
                                                     patch_path=os.path.join(path_grid_raw, 'ovr_patch_order.mat'))
                    ndata_6 = matlab.oversampling_sh(data_6[:,:6],stn_6km_location,lon_goci.shape[0],lon_goci.shape[1],12,
                                                     patch_path=os.path.join(path_grid_raw, 'ovr_patch_order.mat'))
                    ndata_7 = matlab.oversampling_sh(data_7[:,:6],stn_6km_location,lon_goci.shape[0],lon_goci.shape[1],28,
                                                     patch_path=os.path.join(path_grid_raw, 'ovr_patch_order.mat'))
                    ndata_8 = matlab.oversampling_sh(data_8[:,:6],stn_6km_location,lon_goci.shape[0],lon_goci.shape[1],36,
                                                     patch_path=os.path.join(path_grid_raw, 'ovr_patch_order.mat'))
            ...
            print (doy)
            if doy >= 30 & utc == 7:
                fname = f'PM25_RTT_EA6km_{yr}_{doy:03d}_{utc:02d}.csv'
                temp_df = pd.DataFrame(data_stn, columns=header2)
                temp_df.to_csv(os.path.join(path_rtt, 'time_conc','dataset', 'PM25',fname), float_format='%7.7f')
                print (doy)
        else:
            for utc in range(7):
                k=240               
                ...
                high_tmp = extra_samples[extra_samples[:,23]>=240,:]
                ...
                if len(stn_6km)!=0: # no observation data in some utc
                    stn_idx = np.isin(stn_6km_location[:,1], stn_6km[:,12])
                    stn_conc = stn_6km[:,[10,12,0,4,1]] # PM25, stn_num, doy_num,time,yr
                    stn_conc = np.hstack([stn_conc, np.zeros((stn_conc.shape[0], 2))]) 
                    # stn_conc[:,5] = 0 # PM25, stn_num, doy_num,time,yr, ovr
                    stn_conc[:,6] = stn_6km_location[stn_idx,4]# PM25, stn_num, doy_num,time,yr, ovr, stn_location
                    ...
                    high_rate_pre = data_stn[data_stn[:,23]>=240,:].shape[0]/data_stn[:,23].shape[0]*100
                    ...
                    if high_rate_pre<30 & low_rate_pre<30 & (data_stn[:,-1]>1).shape[0]>10000:
                        ...
                        idx = data_stn[:,-2]==1 & data_stn[:,-1]<=8 & data_stn[:,23]>60 & data_stn[:,23]<240
                        data_stn = data_stn[~idx,:]
                    ...
                    if high_rate_pre>30:
                        ...
                        idx = data_stn[:,-1]==1 & data_stn[:,-2]==1 & data_stn[:,23]>=240
                        data_stn = data_stn[~idx,:]
                    if low_rate_pre>30:
                        print (f'low_rate_pre = {low_rate_pre:3.2f} & remove stacked samples \n')
                        idx = data_stn[:,-1]==1 & data_stn[:,-2]==1 & data_stn[:,23]<=60
                        data_stn = data_stn[~idx,:]
                    ...
                    if high_rate<30 & low_rate>30:
                        ...
                        if len(low)!=0:
                            idx_low = np.where(data_stn[:,25] <= low[-1,25] & data_stn[:,23]<=60 & data_stn[:,-1]==1)[0] # remove the low concentration samples under than doy
