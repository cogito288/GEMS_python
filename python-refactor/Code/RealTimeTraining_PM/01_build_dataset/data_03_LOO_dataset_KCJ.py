...
cn_dup_scode2_GOCI6km(:,-1+1:size(jp_dup_scode2_GOCI6km,2))=0
dup_scode2_GOCI6km(:,-1+1:size(jp_dup_scode2_GOCI6km,2))=0
            ...
            for doy = 1:days
                try
                    ...
                    for utc = 0:7
                        ...
                        if i==1
                            data = data(data(:,-1-6)<=1000,:)
                            val_num = 30
                        elif i==2
                            data = data(data(:,-1-6)<=600,:)
                            val_num = 20
                        -1
                        
                        val = data((data(:,-1-4)==doy & data(:,-1-3)==utc & data(:,-1-1)==0),:)
                        val_stn = val(:,-1-5) # stn_num
                        data((data(:,-1-4)==doy & data(:,-1-3)==utc & data(:,-1-1)==0),:) = []
                        ...
                        if size(val_stn,1)>val_num
                            ...
                            for ii= 1:size(num_dist,2)
                                ...
                                if size(num_tmp,1)>val_num
                                    ...
                                    idx_val = matlab.ismember(data(:,-1-5),val_group(:,-1-5))
                                    ...
                                    cal_10_fold = cal_10_fold(:,1:-1-6)
                                    val_10_fold = val_10_fold(:,1:-1-6)
                                    csvwrite_with_headers(strcat(fname,'_LOO_',str(ii,'#03i'),'_cal.csv'),cal_10_fold,header2(1:-1-6))
                                    csvwrite_with_headers(strcat(fname,'_LOO_',str(ii,'#03i'),'_val.csv'),val_10_fold,header2(1:-1-6))
