...
for yr in YEARS: # %2015:2016
	for k in range(10):
        LC_ratio = imread(os.path.join(path_data,'pre','MODIS','MCD12Q1','03_LC_ratio',str(yr), f'EA_{lcclass[k]_ratio_r6_500m_{yr}.tif'))
		...
		vars()[f'LC_{lcclass[k]}'] = LC_ratio
       	matlab.savemat(os.path.join(path,'MODIS_LC_ratio'), f'EA6km_{lcclass[k]}_ratio_r6_{yr}', vars()[f'LC_{lcclass[k]')
