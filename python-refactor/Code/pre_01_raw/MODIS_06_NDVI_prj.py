...
for yr in YEARS:
    ...
    for src_dataset in flist:
        tStart = time.time()
        matlab.check_make_dir(os.path.join(path_myd_processed, '02prj_GCS_WGS84', str(yr))) # debugging
        
        dst_dataset02 = os.path.join(path_myd_processed, '02prj_GCS_WGS84', str(yr), f'p_{os.path.basename(src_dataset)}') # c        
        dst_crs = 'EPSG:4326'
