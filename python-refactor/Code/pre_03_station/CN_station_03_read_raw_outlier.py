...
stn_info_cn = pd.read_csv(os.path.join(path_in_situ, 'AirQuality_China', 'cn_stn_code_lonlat_period.csv'))
...
for yr in YEARS:
    ...
    ndata = matlab.loadmat(os.path.join(path_stn_cn, 'stn_code_data', fname))['stn_CN']
    ...
    for j in range(stn_info_cn.shape[0]):
        ...
        if ((j+1)%100)==0:
            ...
            matlab.savemat(os.path.join(path_stn_cn, fname), {'ndata_scode':ndata_scode})
            ndata_scode = None
    ...
    ndata_scode = None
    file_list = glob.glob(os.path.join(path_stn_cn, f'stn_scode_data_{yr}_*.mat'))
    file_list.sort()
    ...
    matlab.savemat(os.path.join(path_stn_cn,'stn_scode_data', fname), {'ndata_scode':ndata_scode,'header_ndata':header_ndata})
    for fname in file_list: 
        os.remove(fname)
    print (yr)
