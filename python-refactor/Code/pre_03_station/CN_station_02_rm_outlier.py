...
for yr in YEARS:
    ...
    ndata = matlab.loadmat(os.path.join(station_path, 'Station_CN', 'stn_code_data', f'stn_code_data_{yr}.mat'))['stn_yr']
    ...
    for doy in range(1,days+1):
        ...
        if (ndata_temp.shape[0]%nstn_temp==0) and (ndata_temp.shape[0]>=(nstn_temp*4)):
            ...
            th[:,0] =SEM[:,0]+np.nanmean(CO,axis=1)
            th[:,1] =SEM[:,1]+np.nanmean(SO2,axis=1)
            th[:,2] =SEM[:,2]+np.nanmean(O3,axis=1)
            th[:,3] =SEM[:,3]+np.nanmean(NO2,axis=1)
            th[:,4] =SEM[:,4]+np.nanmean(PM10,axis=1)
            th[:,5] =SEM[:,5]+np.nanmean(PM25,axis=1)
            ...
            ndata_temp[:,[6,8,10,12,14,18]]=np.concatenate([PM25.ravel(order='F'),PM10.ravel(order='F'),SO2.ravel(order='F'),NO2.ravel(order='F'),O3.ravel(order='F'),CO.ravel(order='F')], axis=1)
            if stn_CN is None:
                stn_CN = ndata_temp
            else:
